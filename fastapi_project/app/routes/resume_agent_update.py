...
class ResumeAgentUpdateService:
    ...
    async def update_resume_agent(
        ...
        Args:
            payload: 업데이트 요청 데이터 (member_id, answer)
        ...
        self.logger.info(f"이력서 에이전트 업데이트 시작: member_id={member_id}")
        ...
        try:
            ...
            # 5. 결과 처리 및 응답 생성
            response = await self._process_result(member_id, final_state)

            self.logger.info(f"이력서 에이전트 업데이트 완료: member_id={member_id}")
            return response
    ...
    async def _acquire_lock_or_fail(self, member_id: int) -> None:
        """락 획득 또는 예외 발생"""
        lock_acquired = await self.redis_client.acquire_lock(member_id)
        if not lock_acquired:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail=f"회원 {member_id}의 이력서 생성이 이미 진행 중입니다. 잠시 후 다시 시도해주세요.",
            )

    async def _get_and_validate_state(self, member_id: int) -> ResumeAgentState:
        """기존 상태 조회 및 유효성 검증"""
        current_state = await self.redis_client.load_state(member_id)

        if not current_state:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"회원 {member_id}의 이력서 생성 세션을 찾을 수 없습니다. 초기화부터 다시 시작해주세요.",
            )
        ...
        if current_state.info_ready or current_state.step == "completed":
            raise HTTPException(
                ...
                detail=f"회원 {member_id}의 이력서 생성이 이미 완료되었습니다.",
            )
    ...
    async def _process_result(
        self, member_id: int, final_state: ResumeAgentState
    ) -> ResumeAgentUpdateResponse:
        """결과 처리 및 응답 생성"""
    ...
    async def _handle_completion(
        ...
        """이력서 생성 완료 처리"""
        # Redis에서 상태 삭제
        await self.redis_client.delete_state(member_id)

        # S3 object key 생성
        object_key = self._extract_s3_object_key(final_state.docx_path, member_id)

        response = create_agent_update_response(
            member_id=member_id, is_complete=True, resume_object_key=object_key
        )

        self.logger.info(
            f"이력서 생성 완료: member_id={member_id}, object_key={object_key}"
        )
    ...
    async def _handle_next_question(
        ...
        """다음 질문 처리"""
        # Redis에 상태 저장
        save_success = await self.redis_client.save_state(member_id, final_state)
        if not save_success:
            raise HTTPException(
        ...
        response = create_agent_update_response(
            member_id=member_id, is_complete=False, question=next_question
        )

        self.logger.info(
            f"다음 질문 생성: member_id={member_id}, question='{next_question}'"
        )

        return response

    def _extract_s3_object_key(self, docx_path: str, member_id: int) -> str:
        """docx_path에서 S3 object key 추출 또는 생성"""
        if not docx_path:
            # docx_path가 없으면 기본 키 생성
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            return f"resume/member_{member_id}_{timestamp}.docx"
    ...
    async def _safe_release_lock(self, member_id: int) -> None:
        """안전한 락 해제"""
        try:
            await self.redis_client.release_lock(member_id)
        except Exception as e:
            self.logger.error(f"락 해제 실패: member_id={member_id}, error={e}")
...
async def update_resume_agent(payload: ResumeAgentUpdateRequest):
    ...
    try:
        ...
        logger.error(
            f"업데이트 중 예상치 못한 오류: member_id={payload.member_id}, error={str(e)}"
        )
